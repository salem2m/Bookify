    ...
    public class RentalsController : Controller
    {
        ...
        public IActionResult GetCopyDetails(SearchFormViewModel model)
        {
            ...
            if (!copy.IsAvailableForRental && !copy.Book!.IsAvailableForRental)
                return BadRequest(Errors.NotAvilableForRental);

            //check that copy is not on rental
            var isInRentals = _context.RentalCopies.Any(c => c.BookCopyId == copy.Id && !c.ReturnDate.HasValue);
            if (isInRentals)
        ...
        public IActionResult RentalDetails(int id)
        {
            ...
            var viewModel = _mapper.Map<RentalViewModel>(copies);
        ...
        private (string errorMessage, int? maxAllowedCopies) ValidateSubscriber(Subscriber subscriber, int? rentalId = null)
        {
            ...
            var availableCopiesCount = (int)RentalsConfigurations.MaxAllowedRentals - currentRentals;
        ...
        private (string errorMessage, ICollection<RentalCopy> copies) ValidateCopies(IEnumerable<int> selectedSerials, int subscriberId, int? rentalId = null)
        {
            ...
            {
                if (!copy.IsAvailableForRental || !copy.Book!.IsAvailableForRental)
                    return (errorMessage: Errors.NotAvilableForRental, copies);
